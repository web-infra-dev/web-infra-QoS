...
The bottom layer of Rspress is based [Rsbuild](https://github.com/web-infra-dev/rsbuild) for document build process. The Rsbuild can be configured through `builderConfig`. For specific configuration options, please refer to [Rsbuild(https://rsbuild.rs/config/).
